...
| 思考维度 | 内容 |
|----------|------|
| **它为什么存在** | <ul><li>模型回复是流式到达的——用户不想等完整响应才看到输出</li><li>工具执行可以和 API 流并行跑——减少端到端等待时间</li></ul> |
| **它解决什么问题** | <ul><li>Callback 模式的生产者不等消费者——你还没处理完它又推过来了</li><li>Promise 只能返回一个值——中间状态没法表达</li><li>工具顺序执行浪费了 API 流的等待时间——好的调度应该是并行的</li></ul> |
| **它在系统中的位置** | <ul><li>贯穿整个数据管道：API SSE → queryModelWithStreaming → queryLoop → QueryEngine → REPL</li><li>StreamingToolExecutor 在工具执行层和查询循环并行跑</li></ul> |
| **它如何工作** | <ul><li>AsyncGenerator 的 pull 语义自带背压——消费者慢的时候生产者自然停下来等</li><li>StreamingToolExecutor 管工具的四态生命周期——排队、执行、完成、已产出</li><li>并发安全的工具（Read/Glob）可以并行，不安全的（Bash）串行独占</li></ul> |
| **它如何实现** | <ul><li>三层 Generator 管道：queryLoop → query → QueryEngine.submitMessage</li><li>yield* 委托把孩子 generator 的值透给消费者，还能接到 return 值</li><li>Bash 出错了通过 siblingAbortController 中止其他兄弟工具</li></ul> |
| **不同平台如何做** | <ul><li><strong>Cursor Agent</strong>：支持并行工具调用但没有 Generator Pipeline，走 EventEmitter</li><li><strong>OpenClaw</strong>：工具串行执行，没有流式重叠</li><li><strong>Harness Agent</strong>：流水线式执行，不需要终端实时渲染</li></ul> |
| **优势是什么？** | <ul><li><strong>优势</strong><ul><li>AsyncGenerator 天然背压 + 取消 + yield* 组合</li><li>进度消息走 pendingProgress 缓冲区不阻塞工具执行</li><li>新/旧两种执行模式可以渐进式切换</li></ul></li></ul> |
...
- AsyncGenerator 是 Node.js 异步编程中最被低估的 API——它同时实现了惰性求值、背压控制和取消传播。
- Callback 的问题是控制反转（Inversion of Control）：你把回调交给生产者，你就失去了对流程的控制。AsyncGenerator 把控制权还给了消费者。
- 用 for await...of 消费 Generator 时，每次迭代的等待既是生产者产出数据的时间，也是消费者处理的时间——天然背压。
- 对比：事件发射器（EventEmitter）是推模式，AsyncGenerator 是拉模式。推模式在快速生产者面前会把消费者压垮。
...
- SSE（Server-Sent Events）是流式 API 的事实标准。相比于 WebSocket，SSE 是单向的、文本协议的、天然支持断线重连的。
- Claude Code 把 API 流式响应解包成消息类型（text、tool_use、content_block_start 等），每一类都是一个 AsyncGenerator 的 yield。这让消费方可以逐类型处理，不用自己解析流。
- 流式 API 的好处不只是"快"，而是"实时性"——模型还在推理时，用户就能看到 token 逐个出现。这改变了"等待"的心理感受。
- 工具调用块在流中可能是分片到达的，需要收集完整后才能执行。这比普通文本流的处理复杂了一个数量级。
...
- 工具进度流是 Generator Pipe 思想的最佳应用——工具在执行时可以 yield 进度更新，而不需要等完成才报告结果。
- pendingProgress 缓冲区设计解决了一个实时性问题：进度消息可能在 UI 还没准备好渲染时就已经产生了，缓冲区让消息可以暂存。
- 进度流让"长时间运行的工具"变得可以接受——用户看到进度条在走，就不会焦虑地按 Ctrl+C。
- 这是观察者（Observer）和生成器（Generator）模式的融合：工具报告进度，管道传递进度，UI 消费进度——三方松耦合。
...
Generator 组合是 Claude Code 流式架构中最灵活的设计模式——不是通过继承或回调组合代码，而是通过 yield* 组合数据流。

- yield* 委托是 JavaScript 中最被低估的语法特性——它让一个 Generator 可以将自己的产出委托给另一个 Generator，形成无限嵌套的流水线。
- Generator 组合 vs 函数组合：函数组合传递的是返回值，Generator 组合传递的是产出流。前者适合纯计算，后者适合流式处理。
- 废弃（discarding）策略通过封闭执行器而非终止 Generator 来实现"软中止"——Generator 还在运行，但它的产出被静默丢弃了。
- 这种模式的好处是：正在执行的工具不会被粗暴中断，只是其结果不会被消费。这在工具可能产生副作用时尤为重要。
...
背压控制是整个流式架构的"调压阀"——没有背压，快速的生产者会把消费者压垮，导致内存暴涨甚至进程崩溃。

- AsyncGenerator 的 pull-based 语义提供了最高级的背压控制——消费者不拉取下一条，生产者就暂停。这比任何缓存策略都更优雅。
- 隐式背压（pull-based）vs 显式背压（buffer + 水位线）：前者是语言层面的支持，后者是应用层面的 hack。Claude Code 两者都用——Generator 管数据流，buffer 管进度消息。
- Promise.race 唤醒模式允许 Generator 在等待多个异步源时，谁先返回就处理谁——这是"事件驱动"和"生成器"两种模式的融合。
- 双轨设计（StreamingToolExecutor / runTools）的启示：新特性从 feature flag 开始，旧路径长期保留作为安全网——不是"重构"，而是"渐进式迁移"。
