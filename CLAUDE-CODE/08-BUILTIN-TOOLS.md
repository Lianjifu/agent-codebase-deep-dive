...
| 思考维度 | 内容 |
|----------|------|
| **它为什么存在** | <ul><li>看来看去，Claude Code 能干的事全靠这些工具撑着。你点一下鼠标、敲几个字，背后就是某个工具在工作</li><li>每个工具都在安全、性能和体验之间走钢丝——少一个都不行，所有工具合起来才是一个完整的开发助手</li></ul> |
| **它解决什么问题** | <ul><li>Shell 命令是双刃剑——你要让 AI 干活，但怎么保证它不把系统拆了？BashTool 的 18 个文件就是来干这个的</li><li>文件操作看着简单，但写代码的人最怕大段重写。精确替换才是正道，Edit 工具的 old_string 唯一性校验就是这个思路</li><li>代码库大了以后搜索速度就是命门，Ripgrep 加持的 GrepTool 效率是真的可以</li></ul> |
| **它在系统中的位置** | <ul><li>src/tools/ 下面每个子目录就是一个工具，统一在 tools.ts 注册——有了点插件市场的味道</li><li>buildTool 这个工厂函数有意思，默认失败关闭（fail-closed），忘了配安全选项也不会出漏洞</li></ul> |
| **它如何工作** | <ul><li>BashTool 先给命令分个类——是搜索、读取、写入还是破坏性操作，分类不同待遇不同</li><li>FileEdit 要求 old_string 必须唯一匹配，逼你写得更精确而不是模糊替换</li><li>WebFetch 抓了网页先让小模型过一遍，提炼你要的东西再塞回上下文——不浪费宝贵的 token</li></ul> |
| **它如何实现** | <ul><li>BashTool 的 18 个文件各管一摊，从执行到安全到 sed 校验，层层把关</li><li>Read 的 maxResultSizeChars: Infinity 是我觉得最巧妙的——反正结果不入磁盘，不怕读到死循环</li><li>WebSearchTool 不是直接联网搜的，而是又调了一次 API 让模型帮你搜——工具套工具，套娃式实现</li></ul> |
| **不同平台如何做** | <ul><li><strong>Cursor Agent</strong>：没有独立的工具系统，直接撸 VS Code 扩展 API。轻是轻，但能干的事就限制在编辑器范围内了</li><li><strong>OpenClaw</strong>：工具简化为 read/execute/search/edit 五类，安全顾虑少了，但能力边界也小了</li><li><strong>Harness Agent</strong>：工具面向 CI/CD 场景——部署、构建、测试，不需要操心本地文件读写权限</li></ul> |
| **优势是什么？** | <ul><li><strong>优势</strong><ul><li>BashTool 的 18 个文件层层防御，安全设计确实是行业标杆级别的</li><li>三剑客各管一摊：Read 防死循环、Edit 精确替换、Write 打完整 diff，分工清晰</li><li>lazySchema 延迟构建 schema，不用这个工具就不加载它的 schema，启动性能好</li></ul></li></ul> |
...
- BashTool 是 Claude Code 中最复杂也是最危险的单一工具。18 个文件、多层验证流水线——不是过度设计，而是必须。
- 把 50+ 子命令分类到 CommandConfig 的方式值得借鉴：每个命令的授权级别、并发安全性、读写属性都在配置中声明。
- "纵深防御"（Defense in Depth）在这里体现得最充分——不是做一次安全检查，而是 tokenizer → parser → validator → sandbox 层层过滤。
- 有意思的设计：BashTool 不是直接执行字符串命令，而是先走一个完整的解析器（parser），生成 AST，再基于 AST 做安全检查。
...
graph LR
    ...
    R -->|"maxResultSizeChars: Infinity
isConcurrencySafe: true
isReadOnly: true"| RR[读取文件]
    E -->|"maxResultSizeChars: 100K
isConcurrencySafe: false
isReadOnly: false
strict: true"| EE[精确替换]
    W -->|"maxResultSizeChars: 100K
isConcurrencySafe: false
isReadOnly: false"| WW[完全覆写]
...
- FileRead、FileEdit、FileWrite 三个工具的职责划分非常清晰：Read 负责读取（带 offset/limit），Edit 负责精确替换（search/replace 语义），Write 负责全量写入。
- FileEditTool 的 search/replace 模式是 Claude Code 最具辨识度的设计之一——不是传统的"打开文件改第几行"，而是"找到这段内容替换成那段内容"。这更符合 LLM 的认知模式。
- 所有文件工具在写操作前都会走路径安全校验——不是等到 writeFile 才报错，而是调用前就验证路径合法。
- 大文件读取有自动截断机制（max_length 参数），防止工具输出撑爆上下文。
...
sequenceDiagram
    ...
    Grep->>RG: 构建 rg 命令参数
    Note right of Grep: --type ts --glob, 排除 .git 等目录
    RG->>FS: 遍历文件系统
    FS-->>RG: 匹配结果
    RG-->>Grep: 原始输出
...
- Glob 和 Grep 都基于 ripgrep 引擎——速度快、跨平台、忽略 .gitignore 文件。这是工具实现中"选对底层引擎"的典型案例。
- GrepTool 的结果自动做了上下文行展（context_lines），让模型不只能看到匹配行，还能看到前后的背景代码。
- 搜索类工具被标记为 isConcurrencySafe（并发安全），因为多个搜索之间没有副作用，可以同时并行跑。
- ripgrep 的 --json 输出模式让解析变得精确而简单——这比解析纯文本输出优雅得多。
...
WebFetch/WebSearch 这对网络工具展示了 Agent 工具设计中"能力边界"的设定——模型不能直接访问网络，但可以通过工具间接实现。

- WebFetchTool 获取 URL 内容并转为 markdown——这比返回原始 HTML 更节省 token，模型只需要阅读纯文本内容。
- WebSearchTool 把搜索能力封装为工具——模型通过它执行搜索并获取结果摘要。搜索次数限制（max_uses=8）防止无限搜索耗尽预算。
- 域名级别权限控制是网络工具特有的安全需求——不是"能否网络访问"的二元开关，而是"能访问哪些域名"的细粒度规则。
- 对比本地工具（文件读写、命令执行），网络工具的安全模型更接近浏览器——同源策略、请求来源标识、内容安全策略。
...
TaskTools（TaskCreate/TaskStop/TaskStatus）展示了 Agent 系统一个独特的能力——Agent 可以创建和管理子任务，形成任务树。

- TaskCreate 让 Agent 拥有了"创建其他 Agent"的能力——不是直接调工具，而是创建一个新的后台 Agent 来执行任务。这是递归式任务分解在实际工程中的体现。
- 后台任务有独立的生命周期：创建 → 执行 → 状态查询 → 结果收集。Agent 不需要等待任务完成，可以继续处理其他事情。
- lazySchema 延迟初始化在 TaskTools 的 inputSchema 中被广泛使用——不是所有任务都有相同的输入结构，schema 是动态生成的。
- 四个实现模式贯穿所有内置工具：lazySchema（延迟加载）、buildTool 工厂（安全默认）、分离 UI（关注点分离）、权限委托（安全下沉）——这是 40+ 工具的共同架构基因。
