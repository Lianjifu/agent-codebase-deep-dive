...
| 思考维度 | 内容 |
|----------|------|
| **它为什么存在** | <ul><li>1900+ 文件的大型项目，类型系统不只是用来做编译检查的——它是架构级别的约束传播机制</li><li>把运行时报错提前到编译阶段发现，在模块边界强制契约——比代码 Review 靠谱多了</li></ul> |
| **它解决什么问题** | <ul><li>循环依赖是所有大型 TS 项目的噩梦——纯类型文件（types/ 目录）把运行时依赖彻底解了</li><li>SessionId 和 AgentId 都是字符串，传错了编译器不会报错——Branded Types 让编译器帮你抓这种 bug</li><li>TS 的类型在编译后全没了——Zod 在运行时接班做验证</li></ul> |
| **它在系统中的位置** | <ul><li>types/ 是纯类型层，没有运行时代码——谁都能安全引进，不会引起循环</li><li>一个 Zod Schema 搞定三件事：API 契约、运行时验证、类型推断——一个真相来源</li></ul> |
| **它如何工作** | <ul><li>Branded Types 通过交叉类型在编译期区分不同 ID 类型</li><li>DeepImmutable 递归冻结权限上下文等安全关键数据结构</li><li>Zod 的 `z.infer<T>` 让 schema 和类型共享同一真相来源</li><li>tool 泛型通过条件类型区分"属性存在但可选"和"属性存在且必填"</li></ul> |
| **它如何实现** | <ul><li>types/ids.ts 里定义 brand 和窄化函数——asSessionId 是断言式，toAgentId 是安全式，区分得很清楚</li><li>types/permissions.ts 里的纯类型定义被十几个模块 import 但不会循环——因为没运行时代码</li><li>BuiltTool<D> 的类型体操模拟了 spread 操作的类型映射——运行时一行代码，类型系统里写了十几行</li></ul> |
| **不同平台如何做** | <ul><li><strong>Cursor Agent</strong>：直接用 VS Code 插件的类型系统——官方给你啥就是啥，没有品牌类型</li><li><strong>OpenClaw</strong>：普通 TypeScript 类型，不上 Zod 运行时验证</li><li><strong>Harness Agent</strong>：企业项目类型体系完整，也会用 Zod/io-ts 做运行时验证</li></ul> |
| **优势是什么？** | <ul><li><strong>优势</strong><ul><li>纯类型文件优雅破解循环依赖——不加构建步骤</li><li>一个 Zod schema 养三份工——API 契约、运行时验证、类型推断</li><li>ID 混用这种运行时 bug 被提到编译期，开发期就逮住了</li></ul></li></ul> |
...
品牌类型和 DeepImmutable 是编译时安全的"双保险"——前者防止程序员犯错，后者防止程序员不小心。

- SessionId 和 AgentId 都是 string，品牌类型让编译器帮你区分它们——运行时零成本，编译期捕获混乱。
- toAgentId 运行时验证 + asAgentId 断言式转换——两者并存说明了一个道理：外部输入要验证，内部传递靠类型。
- DeepImmutable 递归冻结权限上下文——不是担心开发人员故意改，而是防止大型代码库中有人在角落不小心改了一个字段。
- 这种"使不合法状态不可表示"的理念是类型系统设计的高级境界——不是加更多检查，而是让错误在类型层面就不可能。
...
graph TD
    ...
    style TP fill:#eff6ff,stroke:#2563eb
    style TH fill:#eff6ff,stroke:#2563eb
    style TI fill:#eff6ff,stroke:#2563eb
    style TC fill:#eff6ff,stroke:#2563eb
```
...
types/ 纯类型目录是循环依赖的"外科手术刀"——不改变模块依赖关系，只切断回环的那条边。

- 纯类型文件没有运行时代码，可以被任何模块安全 import——这是破解循环依赖最简单也最优雅的方案。
- `<strong>import type</strong>` 加上 types/ 目录，一个加在导入侧一个加在组织侧——双重保障。
- 向后兼容的重新导出说明了另一个道理：架构重构要考虑现有消费者，不要让他们为你的重构买单。
- PermissionMode 通过 feature() 条件编译控制运行时验证集合，但类型层面始终保留 'auto'——这是一种"编译时剪枝+运行时缩小"的精细控制。
...
graph LR
    ...
    style ZS fill:#fef9c3,stroke:#eab308
```
...
graph TD
    ...
    style Check fill:#fffbeb,stroke:#d97706
```
...
TypeScript 的类型在编译后消失——Zod 在运行时接过了验证的责任。

- 一个 Zod Schema 养三份工（API 契约、运行时验证、类型推断）——"一个真相来源"的设计原则。
- lazySchema 延迟初始化说明 Schema 创建成本虽小，但 40+ 工具累加起来不可忽视。
- 编译时断言（Assert<IsEqual<A, B>>）确保手动类型和 Zod 类型永远一致。
- 工具输入验证不是"要不要检查"，而是"在哪一层检查"——Zod 在边界层而非逻辑层做验证。
...
Zod Schema 和泛型 Tool 类型是运行时与编译时的两次"类型验证"——一个在运行前，一个在运行时。

- 一个 Zod Schema 同时养三份工（API 契约、运行时验证、类型推断）——这不是巧合，是刻意设计的"一个真相来源"。
- lazySchema 延迟初始化说明了一个微妙但重要的事：Schema 的创建成本不是零，在大规模工具系统里会积少成多。
- buildTool 的 fail-closed 安全默认——"假设不可并发、假设有写操作"——体现的是一种防御性设计哲学。
- BuiltTool 的类型体操 `<undefined extends D[K] ? ToolDefaults[K] : D[K]>` 模拟了运行时 spread 的逻辑——类型系统不是万能的，但类型体操可以逼近。
...
判别联合让不合法状态在类型层面就不可表示——这是类型系统设计的高级境界。

- PermissionResult 的 behavior 字段作为判别器，每个分支都是独立类型——编译器强制穷举处理。
- deny 的 decisionReason 必填、allow 的可选——"拒绝必须有理由"的语义通过类型强制执行。
- 10 种 PermissionDecisionReason 变体覆盖了从规则匹配到安全检测的所有场景。
- 类型定义比实现代码更容易膨胀——因为它承载了整个系统的"数据契约"。
...
判别联合和编译时断言是 TypeScript 类型系统中最强大的两个模式——一个做穷举检查，一个做契约验证。

- PermissionResult 判别联合通过 behavior 字段区分 allow/ask/deny/passthrough——每个分支都是独立的类型，消费方必须穷举处理。
- deny 的 decisionReason 必填、allow 的可选——这不是随意设计，而是传递了"拒绝必须有理由"的语义。
- Assert<IsEqual<A, B>> 编译时断言是两套类型定义之间的"诚信检测器"——Zod schema 和手动类型分开维护时，确保它们不会脱节。
- 34 行的 createStore（第 17 章）和 10 种 PermissionDecisionReason 变体的对比说明了同一件事：类型定义比实现代码更容易膨胀——因为它承载了整个系统的"数据契约"。
