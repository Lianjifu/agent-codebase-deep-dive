# 第 17 章：状态管理
> “状态是程序的记忆，而如何组织这份记忆，决定了整个系统的可维护性。”Claude Code 的状态管理体系是一套精心设计的响应式架构。它没有依赖 Redux 等重量级框架，而是从零构建了一个轻量级的 Zustand 风格 Store，配合 React 18 的 `useSyncExternalStore` 实现精准的响应式订阅。本章将深入剖析这一架构的每个层面。

## 引导思考

| 思考维度 | 内容 |
|----------|------|
| **它为什么存在** | <ul><li>大型 React 应用需要统一管状态，但不想用 Redux 那一套重型框架</li><li>34 行代码实现一个够用的 Store——极简主义的典范</li></ul> |
| **它解决什么问题** | <ul><li>全局 UI 状态、会话配置、Agent 状态、权限上下文——这些都要统一管理而不是散落在各处</li><li>文件的读取缓存得有尺寸限制——100 个文件上限、25MB 大小上限，还得 LRU 驱逐</li><li>状态变了要触发副作用（通知远程、写磁盘），又不能把副作用逻辑散到各个 setState 调用中</li></ul> |
| **它在系统中的位置** | <ul><li>Store 在应用内核层，所有组件和工具通过它读写状态</li><li>onChangeAppState 作为统一出口连持久化和远程通知，是状态变更的咽喉要道</li></ul> |
| **它如何工作** | <ul><li>34 行 createStore 实现 Zustand 风格——函数式更新 + Object.is 比较 + Set 做监听器</li><li>useSyncExternalStore 桥接 Store 和 React 渲染循环——React 18 的官方推荐做法</li><li>React Compiler 自动给所有组件加 _c() memoization，开发者不用手写 useMemo</li></ul> |
| **它如何实现** | <ul><li>state/store.ts 里 Set 管监听器，onChange 回调在每次状态变化后被调用</li><li>AppState 用 DeepImmutable 包起来——谁也别想偷改状态</li><li>FileStateCache 用 lru-cache 库做双维度限制——超过 100 条或超过 25MB 就驱逐</li></ul> |
| **不同平台如何做** | <ul><li><strong>Cursor Agent</strong>：用 VS Code ExtensionContext.workspaceState 持久化，没有自己写 Store</li><li><strong>OpenClaw</strong>：没有统一全局 Store，各组件自己 useState/useReducer 管本地状态</li><li><strong>Harness Agent</strong>：React Redux + Toolkit，immer 中间件处理不可变更新</li></ul> |
| **优势是什么？** | <ul><li><strong>优势</strong><ul><li>34 行核心实现极轻量，零外部依赖</li><li>onChangeAppState 集中式副作用消除分散通知的 bug</li><li>React Compiler 自动 memoization 免去手动优化负担</li></ul></li></ul> |
...
1. **条目数上限** (`max: 100`)：最多缓存 100 个文件的状态2. **总大小上限** (`maxSize: 25MB`)：缓存内容的总字节数不超过 25MB`sizeCalculation` 回调使用 `Buffer.byteLength(value.content)` 计算每个条目的实际内存占用。`Math.max(1, ...)` 确保空文件也占据至少 1 字节的配额，避免除零错误。
...
flowchart TB
    ...
    subgraph "新架构（集中通知）"
        Q1["Shift+Tab"] --> QS["Store"]
        Q2["ExitPlanMode"] --> QS
        Q3["/plan 命令"] --> QS
        Q4["rewind"] --> QS
        Q5["Bridge"] --> QS
        QS --> |"onChangeAppState"| CCR2["CCR / SDK"]
    end
```
