...
| 思考维度 | 内容 |
|----------|------|
| **它为什么存在** | <ul><li>单线程的 Agent 能干的事有限——复杂任务（比如 /batch 的多文件重构）就得拆成子任务并行跑</li><li>Fork 机制受 Unix fork() 启发——子进程继承父进程的完整上下文，但又各自独立</li></ul> |
| **它解决什么问题** | <ul><li>并行执行：后台放好几个 Agent 同时干活，做完了一起汇报——batch 模式就是这么来的</li><li>上下文隔离：子 Agent 改了什么不会影响父 Agent 的状态，互不干扰</li><li>中断恢复：Task 后台任务可以从 sidechain transcript 里恢复——写了一半被人关了也能接上</li></ul> |
| **它在系统中的位置** | <ul><li>位于 L2 Agent 运行时层，AgentTool 作为工具出现在 L3 的工具列表里</li><li>createSubagentContext 是上下文隔离的核心——默认隔离、显式共享</li></ul> |
| **它如何工作** | <ul><li>Fork 的 model: 'inherit' 确保子 Agent 和父代用同一个模型，共享 prompt cache 前缀</li><li>buildForkedMessages 构建子代的初始消息，所有 Fork 共享字节完全一致的前缀来最大化缓存命中</li><li>SendMessage 通过 name-to-id 路由表找到正在运行的后台 Agent 并给它发指令</li></ul> |
| **它如何实现** | <ul><li>forkSubagent.ts 中的 isForkSubagentEnabled 受 feature gate 控制，不是什么时候都可用</li><li>Task 后台 Agent 通过 shared/spawnMultiAgent.ts 启动，runAsyncAgentLifecycle 管理完整生命周期</li><li>createSubagentContext 创建隔离的子 Context，setAppState 不透传到父 Agent</li></ul> |
| **不同平台如何做** | <ul><li><strong>Cursor Agent</strong>：不支持多 Agent 并发，所有操作在主对话线程里线性执行</li><li><strong>OpenClaw</strong>：原生支持分布式 Agent 网络，Agent 可以跨机器通信和动态组队，但没有 Fork 这种轻量上下文克隆</li><li><strong>Harness Agent</strong>：Pipeline 支持并行 Step，但每个 Step 运行在隔离的工作环境中，不涉及对话上下文的复制</li></ul> |
| **优势是什么？** | <ul><li><strong>优势</strong><ul><li>Fork 的 prompt cache 共享大幅降低 API 成本——并行 Fork 的边际成本很低</li><li>Task 后台 Agent 可以持久化和恢复，适合跑长时间的任务</li><li>createSubagentContext 默认隔离、显式共享的原则在安全和灵活之间取得了好平衡</li></ul></li></ul> |
...
sequenceDiagram
    ...
    Note over Fork1,Fork2: 共享前缀（history + assistant + placeholder_results）
    Note over Fork1,Fork2: prompt cache 命中！
...
- Fork 是 Claude Code 最核心的并发原语——它不是一个函数，而是一个模式：复制当前上下文，在新的子 Agent 中并行执行任务。
- Fork 的 prompt cache 共享是最精妙的设计：父 Agent 已经建立了 prompt cache，子 Agent Fork 直接继承——不花额外成本就享受了 cache 加速。
- 上下文隔离 vs 共享的平衡：createSubagentContext 默认隔离（setAppState），显式共享（setAppStateForTasks）——原则是"默认安全，按需开放"。
- Fork 是一个操作但有两层含义：逻辑上是"开新任务"，物理上是"复制上下文"。理解这两层含义的不同，就理解了 Fork 的设计精华。
...
sequenceDiagram
    ...
    Resume->>Resume: filterUnresolvedToolUses, filterOrphanedThinkingOnlyMessages, reconstructForSubagentResume
...
- Resume 是 Fork 的逆操作——把之前 Fork 出去的上下文合并回来。没有 Resume，Fork 就变成了 fire-and-forget。
- Token 预算在 Resume 时做检查是必要的：子 Agent 消耗的 token 不能超过父 Agent 的预算约束，否则父 Agent 自己的查询可能因为 token 不够而失败。
- 选择性恢复（只恢复特定 Agent 的结果）比全量恢复更实用——在实际场景中，你通常只关心某个子任务的结果，而不是所有子任务。
- Resume 和 Fork 是一对设计模式：Fork 分出去，Resume 收回来——这个来回的过程就是"分治"思想在 Agent 系统中的实现。
...
(AbortError)
    ...
    Completed --> Notified: enqueueAgentNotification
(status: completed)
    Killed --> Notified: enqueueAgentNotification
(status: killed)
    Failed --> Notified: enqueueAgentNotification
(status: failed)
    ...
    state Running {
        ...
        Processing --> Summarizing: startAgentSummarization
(可选)
...
后台任务让 Agent 拥有了"异步执行"的能力——不是所有任务都需要即时完成。

- 生命周期：创建 → 执行 → 状态查询 → 结果收集——Agent 不需要等待。
- LocalAgentTask 和 RemoteAgentTask 共享同一接口——本地和远程对调用方透明。
- 持久化是关键设计——即使主 Agent 重启，后台任务状态仍然保留。
- 错误处理比同步更复杂——Agent 需要主动查询或通过回调被通知。
...
agentMemory.ts 是 Agent 的"短期记忆"——运行时的上下文快照。

- 快照（snapshot）在 Fork 时保存上下文，Resume 时恢复——全量快照简单但昂贵。
- 运行时的上下文快照和跨会话持久化记忆是两回事——前者在内存中，后者需要存储。
- "没有标准的跨 Agent 共享记忆机制"是已知限制——每次 Fork 都是完整上下文复制。
- 快照的粒度决定 Fork 成本——这是简单性和效率的经典权衡。
...
SendMessageTool 是 Agent 间通信的桥梁——一个 Agent 可以向另一个 Agent 发送消息。

- Agent 间通信使用工具调用的形式——不是独立的通信协议，而是复用现有的 Tool 机制。
- 消息的目标 Agent 通过 Agent ID 指定——和进程间通信中指定目标 PID 类似。
- 通信是异步的——发消息的 Agent 不等回复即可继续工作。
- 通信的可观测性：所有跨 Agent 消息都被记录在 transcript 中，可以回放审查。
...
子 Agent 编排是 Claude Code"分治"思想的工程实现——把大任务拆成小任务，并行或异步执行。

- Fork 是最核心的原语——复制上下文、隔离执行、并行运行。
- Resume 是 Fork 的逆操作——把子任务的结果合并回来。
- 后台任务让 Agent 可以"边做边等"——异步处理耗时操作。
- Agent 记忆和通信让子 Agent 不再是孤立的执行单元——它们可以共享信息和协调行动。
