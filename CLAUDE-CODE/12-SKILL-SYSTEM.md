...
| 思考维度 | 内容 |
|----------|------|
| **它为什么存在** | <ul><li>Skill 是 Claude Code 里最高层次的抽象——站在 Agent、工具、MCP 之上，把多步工作流封装成一键触发的 /command</li><li>用户看到的 /commit、/review-pr 这些斜杠命令，背后是一套完整的发现、解析和执行流水线</li></ul> |
| **它解决什么问题** | <ul><li>/batch 把 Plan 模式 + 并行 Agent + Simplify 递归调用串在一起——用户一句话就搞定复杂工作流</li><li>/claude-api 根据项目语言自动注入对应的 API 文档——检测到 Python 就加载 Python SDK 文档</li><li>Inline 模式轻量注入当前上下文，Fork 模式启动独立子 Agent——按任务复杂度自动选</li></ul> |
| **它在系统中的位置** | <ul><li>Skill 没有自己的独立类型——它就是 Command 的 type: 'prompt' 子集</li><li>通过 SkillTool 注入会话，三种来源（文件系统 + 内置 + MCP）聚合并通过统一接口暴露</li></ul> |
| **它如何工作** | <ul><li>Inline 模式把 Skill 提示词直接注入主对话，主 Agent 在当前轮次就把活干了</li><li>Fork 模式通过 prepareForkedCommandContext 创建隔离的子 Agent 来执行</li><li>Skill 列表展示限制在上下文窗口的 1% 以内——不是所有 Skill 都显示，而是按优先级和相关性筛选</li></ul> |
| **它如何实现** | <ul><li>bundledSkills.ts 注册内置 Skill，loadSkillsDir.ts 扫描文件系统发现用户自定义 Skill</li><li>SkillTool 的 executeForkedSkill 构建 Agent 定义和提示词消息</li><li>/loop Skill 用的"Prompt as Controller"模式——不是直接操作 Cron 系统，而是生成指令让模型调用 ScheduleCronTool</li></ul> |
| **不同平台如何做** | <ul><li><strong>Cursor Agent</strong>：没有 Slash 命令或技能系统，多步工作流通过 Composer 面板手动编排</li><li><strong>OpenClaw</strong>：功能封装在 Agent 定义中而非独立的能力单元，Agent 间通过消息传递协作</li><li><strong>Harness Agent</strong>：Pipeline 模板提供类似技能的可复用能力包，但编排方式是声明式 DAG 而非模型自主调用</li></ul> |
| **优势是什么？** | <ul><li><strong>优势</strong><ul><li>Command 统一抽象让 Skill 无缝集成到现有命令系统中，用户感知一致</li><li>Inline/Fork 双模根据任务复杂度自动选择执行策略</li><li>Prompt as Controller 模式有极高的灵活性和可组合性</li></ul></li></ul> |
...
- Skill 和 Agent 的本质区别不在于功能，而在于触发方式——Agent 是预先定义好的角色，Skill 是用户通过 /command 触发的能力。
- Prompt 模板 + Agent 封装的组合让 Skill 既足够灵活（提示词决定行为），又足够规范（Agent 提供安全边界和工具绑定）。
- Skill 的"模板"属性是其最大的优势：一个写得好好的 Skill prompt，换个项目换个语境也能复用——只需要调整模板参数即可。
- Command 统一抽象意味着：Skill、插件、内置命令，在用户视角都是用 /slash 触发的——感知一致性降低学习成本。
...
flowchart TD
    subgraph "文件系统来源"
        FS1["~/.claude/skills/*.md
(用户级)"]
        FS2[".claude/skills/*.md
(项目级)"]
        FS3["<managed>/.claude/skills/*.md
(策略级)"]
    end

    subgraph "代码注册来源"
        B1["initBundledSkills()
(内置 Skill)"]
    end
...
- Skill 发现机制支持三种来源（文件系统、插件、MCP）——这是"可插拔"架构在能力系统中的应用：不用手动注册，放到约定目录就能自动发现。
- 文件系统是最主要的 Skill 来源：用户把 .md 文件放到约定目录下，系统自动识别为 Skill。这比 GUI 管理或数据库存储更符合开发者的工作流。
- MCP Skill 是 MCP 协议扩展能力的一个有趣应用——远程 Skill 通过 MCP 协议加载，让能力发现超越了本地文件系统。
- 三种来源的优先级决定了覆盖顺序：本地覆盖远程，项目覆盖全局——和 Agent 的优先级策略一脉相承。
...
executeForkedSkill 通过 Fork 子 Agent 模式执行 Skill——每个 Skill 在隔离上下文中运行。

- Fork 模式隔离了执行环境——工具调用不影响主对话的消息列表。
- Inline 模式在同一个上下文中执行——适合简单、快速的 Skill，不需要 Fork 开销。
- Fork/Inline 的选择根据 Skill 复杂度自动决定——系统判断而非用户选择。
- Skill 的核心哲学：可组合的能力单元，不是可执行的任务脚本。
...
内置 Skill 是 Claude Code 内置的"瑞士军刀"——覆盖了最常见的使用场景。

- 内置 Skill 包括代码审查、架构分析、测试生成、文档撰写等——每个都有精心设计的 Prompt 模板。
- 内置 Skill 的 Prompt 经过了大量迭代优化——不是一次写好的，而是在真实使用中逐步打磨的。
- 内置 Skill 和自定义 Skill 使用同一执行机制——用户定义的 Skill 拥有和内置 Skill 一样的运行时支持。
- Skill 的执行可以带参数（通过 /skill_name arg1 arg2 语法）——不是简单的"执行 Skill"，而是"执行 Skill 并传入上下文"。
...
Skill 和插件（Plugin）在概念上有重叠，但机制不同——Skill 是 Prompt 模板，插件是可执行代码包。

- Skill 是"提示词 + Agent 封装"——本质上是给模型的一套指令，不需要编译或打包。
- 插件是"可执行代码"——需要编译、安装、加载，可以包含复杂的逻辑和外部依赖。
- Skill 比插件更轻量——创建 Skill 只需要写一个 .md 文件，创建插件需要写代码并发布。
- MCP 协议桥接两者——MCP 插件可以暴露 Skill，Skill 也可以调用 MCP 工具。
...
Skill 系统是 Claude Code"能力扩展"的用户界面——不是改代码加功能，而是写 Prompt 加能力。

- 发现机制（文件系统自动扫描）让添加 Skill 零配置——放到约定目录就能用。
- Fork/Inline 双执行模式兼顾隔离和效率——复杂 Skill 有隔离保护，简单 Skill 有极低延迟。
- Skill 和 Agent 的异同：两者都是"带提示词的能力单元"，Agent 更偏角色定义，Skill 更偏任务定义。
- MCP 插件的未来：如果 MCP 生态成熟，Skill 可以从本地 Prompt 扩展到远程能力。
